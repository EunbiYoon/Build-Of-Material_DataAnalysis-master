...
###########################GERP###############################
gerp=pd.read_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/gerp.xlsx')
# #파일에 따라 추가하기
...
gerp.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/gerpresult.xlsx')

###########################NPT###############################
npt=pd.read_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/npt.xlsx')
...
npt.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/nptresult.xlsx')
...
for i in range(len(gerp)):
    gerp_parent=gerp.at[i,"Parent Item"]
    if gerp_parent[:1]=="RV13D1AMAZU.ABWEUUS":
        gerp_parent="TAV35812101"
    gerp_part=gerp.at[i,"Child Item"]
    gerp_subpart=str(gerp.at[i,"Child Item"])[:-2]
    gerp_des=gerp.at[i,"Description"]
    gerp_qty=gerp.at[i,"Qty Per Assembly"]
    remain_seq=gerp.at[i,"Seq"]

    for j in range(len(npt)): #duplicate 제외
        npt_part=npt.at[j,"Part No"]
        npt_level=npt.at[j,"level"]
        npt_subpart=str(npt.at[j,"Part No"])[:-2]
        npt_parent=npt.at[j,"Parent Part"]
        npt_des=npt.at[j,"Desc."]
        npt_qty=npt.at[j,"Unit Qty"]
        match_number=npt.at[j,"Seq."]

        ############### Screw,Tapping ###############
        if npt_des=="Screw,Tapping" and gerp_des==npt_des and gerp_part==npt_part and gerp_qty==npt_qty and gerp_parent==npt_parent: 
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #Screw,Taptite 
        elif npt_des=='Screw,Taptite' and gerp_parent==npt_parent and gerp_qty==npt_qty and gerp_des==npt_des: 
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1
            print(remain_seq)
        ...
        #Screw,Customized
        elif npt_des=="Screw,Customized" and gerp_des==npt_des and gerp_part==npt_part and gerp_qty==npt_qty and gerp_parent==npt_parent:
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #Duct Assembly
        elif npt_des=='Duct Assembly' and npt_part.__contains__(gerp_subpart) and gerp_parent==npt_parent: # 끝자리 한자리까지 일치
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #Packing
        elif npt_des=='Packing' and gerp_des==npt_des and gerp_part==npt_part: #부모랑 des일치하면 우선 배정, 파트넘버 전부 같음
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #Resin,EPS
        elif npt_des=='Resin,EPS' and gerp_des==npt_des and gerp_part==npt_part and gerp_parent==npt_parent: #부모랑 des일치하면 우선 배정, 파트넘버 전부 같음
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #Upper Hanger
        elif gerp_des=='Hanger,Upper' and gerp_des==npt_des and gerp_subpart==npt_subpart: 
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #door glass -> parent 다른게 두게
        elif gerp_des=="Door,Glass" and gerp_des==npt_des and gerp_subpart==npt_subpart:
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1

        #holder -> parent 다른게 두게
        elif npt_des=="Holder" and npt_level>2 and gerp_des==npt_des and gerp_part==npt_part:
            match_list.at[match_number,"gerp_unique"]=remain_seq
            unique_match.at[count,"index"]=i
            count=count+1
...
#remain_match 존재 유무
unique_match_count=len(unique_match)
...
for i in range(len(remain_gerp)): #i -> gerp
    ...
    if remain_parent[:1]=="RV13D1AMAZU.ABWEUUS":
        remain_parent="TAV35812101"
    remain_subparent=remain_parent[:-2]
    ...
    for j in range(len(npt)):
        ...
        ############### Heater Assembly ###############
        elif remain_des=="Heater Assembly" and npt_des==remain_des:
            match_list.at[match_number,"gerp_re"]=remain_seq
            remain_match.at[count,"index"]=i
            count=count+1

        # ############### Heater Bracket ###############
        # elif remain_des=="Bracket,Heater" and npt_des=='Heater Assembly':
...
remain_gerp.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/remaingerp.xlsx')
...
match_list.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/matchlist.xlsx')
...
#save file
match_join.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/matchjoin.xlsx')
...
#######################Add MTL Column####################
mtl=pd.read_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/gerp.xlsx',sheet_name='MTL Cost')
...
############## 1- Coil, Steel(STS) / Sheet,Steel(GI) ##############
...
for i in range(len(final_table)):
    ...
    if part_des.__contains__('Tub,Drum(') and price_match=='Substitute':
        for j in range(len(final_table)):
            ...
            if another_des=='Coil,Steel(STS)' and another_parent==child_part and another_match!=("Substitute"):
                final_table.at[i,"price match"]=(final_table.at[i,"Net Material"]-another_price)*final_table.at[i,"Qty Per Assembly"]
    elif part_des.__contains__('Cover,Rear') and price_match=='Substitute':
        for j in range(len(final_table)):
...
final_table.to_excel('C:/Users/RnD Workstation/Documents/NPTGERP/0421/DR/final_table2.xlsx')